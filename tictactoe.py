...
#board_state = TicTacToeGameState(state=c_board, next_to_move=1、-1)
class TicTacToeGameState(object):
    ...
    def move(self, move):
        ...
        next_to_move = TicTacToeGameState.o if self.next_to_move == TicTacToeGameState.x else TicTacToeGameState.x #x=1 o =-1区分玩家和ai
        return TicTacToeGameState(new_board, next_to_move)

    def get_legal_actions(self):
        indices = np.where(self.board == 0)#所有为0（空）的位置
